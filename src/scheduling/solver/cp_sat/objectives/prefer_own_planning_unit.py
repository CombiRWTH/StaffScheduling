...
from scheduling.domain.planning_unit import PlanningUnitMembership, PlanningUnitType, PlanningUnit, PlanningUnitId
...
class PreferOwnPlanningUnit:
    ...
    def add_to_model(
        ...
        for key, variable in ctx.assignment_variables.items():
            ...
            if employees_PU_dict[employee_id] != planning_unit_id and employees_PU_dict[employee_id] not in shared_pool_ids:
                num_not_preferred_planning_unit += 1

        prefer_own_planning_unit_penalty = ctx.model.new_int_var(
            0,
            100000,
            "not_preferred_planning_unit"
        )

        ctx.model.add(prefer_own_planning_unit_penalty == num_not_preferred_planning_unit).with_name("prefer_own_planning_unit__penalty")
