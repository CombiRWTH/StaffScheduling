    ...
    def create(self, model: CpModel, variables: dict[str, IntVar]):
        ...
        for employee in self._employees:
            ...
            while i < len(self._days) - 2:
                ...
                model.Add(variables[key1] == 3).OnlyEnforceIf(
                    is_night1
                )  # shift 3 = night
                ...
                night_phase = model.NewBoolVar(
                    f"night_phase_{employee.get_id()}_{day1}"
                )
                model.AddBoolAnd([is_night1, is_night2]).OnlyEnforceIf(night_phase)
                model.AddBoolOr([is_night1.Not(), is_night2.Not()]).OnlyEnforceIf(
                    night_phase.Not()
                )
                ...
                if i + 3 < len(self._days):
                    ...
                    is_both_free = model.NewBoolVar(
                        f"is_both_free_{employee.get_id()}_{day3}_{day4}"
                    )
                    ...
                    model.AddBoolOr([is_free3.Not(), is_free4.Not()]).OnlyEnforceIf(
                        is_both_free.Not()
                    )

                    penalty_var = model.NewBoolVar(
                        f"penalty_after_night_phase_{employee.get_id()}_{day1}"
                    )
