        ...
        for employee in self._employees:
            for day in self._days[:-1]:
                for station in self._stations:
                    for station_tomorrow in self._stations:
                        night_shift_today_variable = shift_assignment_variables[employee][day][
                        ...
                        night_shift_tomorrow_variable_special = shift_assignment_variables[employee][
                            day + timedelta(1)
                        ][self._shifts[SPECIAL_NIGHT_SHIFT_INDEX]][station_tomorrow]

                        day_tomorrow_variable = employee_works_on_day_variables[employee][day + timedelta(1)]
                        # where are day_tomorrow_variables enforced?
                        # this may be the cause of the bug menitioned in the docs
