...
class ShiftAssignmentVariables:
    ...
    def __init__(self, internal_dict: dict[int, dict[Day, dict[int, dict[Station, Variable]]]]):
    ...
    def __len__(self) -> int:
        ...
        for day_dict in self._data.values():
            for shift_dict in day_dict.values():
                for station_dict in shift_dict.values():
                    count += len(station_dict)
        return count


class DayShiftDict:
    """Intermediate dictionary for Day -> Shift -> Station -> Variable mapping."""

    def __init__(self, day_dict: dict[Day, dict[int, dict[Station, Variable]]]):
        self._data = day_dict
...
class ShiftDict:
    """Intermediate dictionary for Shift -> Station -> Variable mapping."""

    def __init__(self, shift_dict: dict[int, dict[Station, Variable]]):
        self._data = shift_dict

    def __getitem__(self, shift: Shift) -> "StationDict":
        return self._data[shift.get_id()]


class StationDict:
    """Final dictionary for Station -> Variable mapping."""

    def __init__(self, station_dict: dict[Station, Variable]):
        self._data = station_dict

    def __getitem__(self, station: Station) -> Variable:
...
def create_shift_assignment_variables(
    employees: list[Employee], days: list[Day], shifts: list[Shift], stations: list[Station], model: CpModel
) -> ShiftAssignmentVariables:
    vars: dict[int, dict[Day, dict[int, dict[Station, Variable]]]] = {}
    for employee in employees:
        ...
        for day in days:
            vars[e_key][day] = {}
            for shift in shifts:
                vars[e_key][day][shift.get_id()] = {}
                for station in stations:
                    vars[e_key][day][shift.get_id()][station] = model.new_bool_var(
                        f"({e_key}, '{day.strftime('%Y-%m-%d')}', {shift.get_id()}, {station})"
                    )
    ...
    for employee in employees:
        for day in days:
            model.add_max_equality(
                ...
                [shift_assignment_vars[employee][day][shift][station] for shift in shifts for station in stations],
            )
