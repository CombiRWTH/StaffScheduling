...
def add_free_days_near_weekend(
    ...
    for n in range(num_employees):
        for d in range(num_days - 1):
            ...
            model.Add(work[(n, d)] + work[(n, d + 1)] != 0).OnlyEnforceIf(rest_pair.Not())
        ...
        for d in range(1, num_days - 1):
            ...
            model.Add(work[(n, d + 1)] == 1).OnlyEnforceIf(solo_rest)
            model.Add(sum([work[(n, d - 1)], work[(n, d)], work[(n, d + 1)]]) == 2).OnlyEnforceIf(solo_rest)
            model.Add(sum([work[(n, d - 1)], work[(n, d)], work[(n, d + 1)]]) != 2).OnlyEnforceIf(solo_rest.Not())
            objective_terms.append(-solo_rest)  # Extra points when there is no isolation
